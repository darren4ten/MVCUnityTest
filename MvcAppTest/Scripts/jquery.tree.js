    ...
    $.fn.treeview = function (settings) {
        ...
        function buildnode(nd, ht, deep, path, isend) {
            ...
            if (deep == 1) {
                ...
                if (nd.parent && nd.parent.isend) {
                    ht.push("<img class='bbit-tree-ec-icon ' src='" + dfop.cbiconpath + "s.gif'/>");
                } else {
                ...
                if (nd.parent) {
                    ...
                    //ht.push("<img class='bbit-tree-icon  bbit-tree-ec-icon' src='" + dfop.cbiconpath + "s.gif'/>");
                    var ndParent = nd.parent;
                    var parentArr = [];
                    while (ndParent) {
                        if (ndParent.isend) {
                            parentArr.push("<img class='bbit-tree-ec-icon ' src='" + dfop.cbiconpath + "s.gif'/>");
                        } else {
                            parentArr.push("<img class='bbit-tree-elbow-line' src='" + dfop.cbiconpath + "s.gif'/>");
                        }
                        ndParent = ndParent.parent;
                    }
                    for (var d = parentArr.length - 1; d >= 0; d--) {
                        ht.push(parentArr[d]);
                    }

                } else {
                    ht.push("<img class='bbit-tree-icon bbit-tree-elbow-line' src='" + dfop.cbiconpath + "s.gif'/>");
